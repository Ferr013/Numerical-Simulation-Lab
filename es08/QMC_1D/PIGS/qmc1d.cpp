...
//REPLACED ROOT WITH MY RANDOM CLASS
...
double variationalWaveFunction(double v)
{
    ...
    double sigma = 0.61;
    double result;
    result = exp(-pow(v-mu,2.)/(2*sigma*sigma)) + exp(-pow(v+mu,2.)/(2*sigma*sigma));
    return result;
}
...
double variationalWaveFunction_second(double v)
{
    ...
    double sigma = 0.61;
    double result;
    result = exp(-pow(v-mu,2.)/(2*sigma*sigma))*(pow((v-mu)/(sigma*sigma),2.)-1./(sigma*sigma)) + exp(-pow(v+mu,2.)/(2*sigma*sigma))*(pow((v+mu)/(sigma*sigma),2.)-1./(sigma*sigma));
    return result;
}

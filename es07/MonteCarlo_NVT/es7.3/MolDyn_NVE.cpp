...
void GetGave(void){
  ...
  for (int i_bin = 0; i_bin < nbins; i_bin++){
    err_gdir=Error(glob_av[igofr+i_bin],glob_av2[igofr+i_bin],iblock);
    Gave << setw(wd)<< i_bin << setw(wd)  << glob_av[igofr+i_bin]/(double)iblock <<  setw(wd) << err_gdir <<  endl;
  }
  Gave.close();
}
